...
\tiny 
web-page: \url{www.giannidiorestino.it} (slides) 
...
\end{frame}
    ...
    \begin{equation*}
        u(x_1,x_2) = u_0 + \left(u_1(x_1) + u_2(x_2)\right) + u_{12}(x_1,x_2)
    \end{equation*}
    \item Notice that we do not require $u_1 \perp u_2$ as it is done in Hajek:1968 and Sobol':2001. Cf. also Efron and Stein 1981.
...
\begin{equation*}
    ...
    \end{equation*}
  \item Cf. Lauritzen 1996 and Sergeant-Pethuis 2021. 
...
\item  Each $I \neq \emptyset$ is an \emph{interaction}. 
   Let $X_I$ be the components projection on $I$, $X_I = (X_j \colon j \in I)$.
    ...
    with 
    \begin{equation*}
        \expectat q {f_1\circ X_1} = \expectat {q_1} {f_1} = 0 \ , \quad
...
\end{equation*}
\item That is, the complement fibre $R_q \openplan{q_1}{q_2}$ is 
\begin{multline*}
\hajekof{q}\expfibreat q \opensimplexon{\Omega_1 \times \Omega_2} =  H_1(q) =  \\ \setof {f_1 \circ X_1 + f_2 \circ X_2}{\expectat {q_1}{f_1} = \expectat {q_2}{X_2} = 0} \ ,
 \end{multline*}
  ...
  \end{equation*}
\item The splitting suggests the parameterisation of each $q \in \opensimplexon{\Omega_1 \times \Omega_2}$ by the margins and an interaction. \end{itemize}
\end{frame}

\begin{frame}[plain]\small\frametitle{$\openplan{q_1}{q_2}$ as an affine space}
\begin{itemize}
...
\end{equation*}
\item The \emph{dual transport} is computed as follows. If $q, r \in \openplan {q_1}{q_2}$, $c_{12} \in S_q \openplan {q_1}{q_2} = H_2(q)$, and $d_{12} \in S_r \openplan {q_1}{q_2} = H_2(r)$, then 
\begin{gather*}
  \scalarat r { \mtransport q r c} d = \expectat q {cd} = \scalarat q c {d - \hajekof q d} = \scalarat q c {\left(\mtransport q r\right)^T d} \\
  \text{that is,} \quad \left(\mtransport q r\right)^T = \left(I - \hajekof q\right) \ .
\end{gather*}
\end{itemize}
\end{frame}
...
\begin{itemize}  
\item Let us compute the mixture geodesic. If $(q,c) \in S \openplan {q_1}{q_2}$, an \emph{m-geodesic} is a curve in $t \mapsto q(t) \in \openplan {q_1}{q_2}$ with ``constant'' velocity.
\item Let $(q(0),\velocity q(0)) = (q,c)$ and $\velocity q(t) = \mtransport q {q(t)} c$. It follows
\begin{equation*}
...
\item The \emph{affine chart} is the geodesic at $t=1$:
\begin{equation*}
...
\end{equation*}
\item A solution of this equation requires the computation of the Hajek projection.
\end{itemize}
...
that is,
\begin{equation*}
  \Grad C(q) = \left(c - C(q)\right) - \hajekof {q} \left(c - C(q)\right)
\end{equation*}
\end{itemize}
\end{frame}
...
\item The form of the gradient is compatible with the classical result in OT: if $\bar q$ is an optimal plan, that the cost is equal to the sum of two univariate potentials. Cf., e.g., Peyr\'e and Cuturi 2019.
\end{itemize}
\end{frame}
