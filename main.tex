...
\begin{frame}[plain]\small\frametitle{Transport plans in $\opensimplexon{\Omega_1 \times \Omega_2}$}
 \begin{itemize}
     ...
     \begin{equation*}
         \openplan{q_1}{q_2} = \setof{q \in \opensimplexon{\Omega_1 \times \Omega_2}}{X_1 q = q_1,X_2 q= q_2}
     \end{equation*}
...
\item Let $t \mapsto q(t)$ be a smooth curve of $\opensimplexon {\Omega_1 \times \Omega_2}$ with values in the set of strictly positive transport plans, 
  ...
  \item Hence $\condexpat {q(t)} {\velocity q(t)} {X_j} = 0$, $j=1,2$.
 That is, \emph{$\velocity q(t)$ is a  $q(t)$-interaction}, $\velocity q(t) \in H_2(q(t))$.
\item Conversely, let $q \in \openplan{q_1}{q_2}$ and $c_{12} \in H_2(q)$. The curve $t \mapsto (1+tc_{12}) \cdot q$ is defined for $t$ in a neighborhood of 0 and stays in $\openplan{q_1}{q_2}$.
\begin{equation*}
...
\end{itemize}
\begin{block}{}For all $q \in \openplan{q_1}{q_2}$, the velocities' fibre is the vector space of interactions,
\begin{equation*}
