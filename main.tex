...
\huge PART 1 \\ ANOVA and STATISTICAL BUNDLE.
...
\begin{frame}[plain]\small
  ...
  \end{equation*}
\item The \emph{velocity} of a smooth curve $t \mapsto q(t) \in \opensimplexon \Omega$ is the curve $t \mapsto (q(t), \velocity q(t)) \in \expbundleat {\opensimplexon \Omega}$ defined by
  \begin{equation*}
...
\begin{itemize}
    \item Assume a product sample space $\Omega = \Omega_1 \times \Omega_2$ and consider the probability simplex $\simplexon{\Omega_1 \times \Omega_2}$. The two marginalisation mappings are
    \begin{align*}
 X_1& \colon \simplexon{\Omega_1 \times \Omega_2} \ni q(\cdot,\cdot) \mapsto \sum_{x_2} q(\cdot,x_2) \in  \simplexon{\Omega_1}  \\
 X_2& \colon \simplexon{\Omega_1 \times \Omega_2} \ni q(\cdot,\cdot) \mapsto \sum_{x_1} q(x_1,\cdot) \in  \simplexon{\Omega_2} 
    \end{align*}
...
\begin{frame}[plain]\small\frametitle{Transport plans in $\opensimplexon{\Omega_1 \times \Omega_2}$}
 \begin{itemize}
     ...
     \begin{equation*}
         \overset{\circ}{\Pi}(q_1,q_2) = \setof{q \in \opensimplexon{\Omega_1 \times \Omega_2}}{X_1 q = q_1,X_2 q= q_2}
     \end{equation*}
 
 \item \emph{$\overset{\circ}{\Pi}$ is a sub-manifold of the affine statistical manifold on $\maxexpat {\Omega_1 \times \Omega_2}$}.
...
\item Basic examples of sub-manifolds of the affine statistical manifold are exponential families and mixture models and exponential families. Here we discuss the set of transport plans which turns out to be a mixture model.
 \end{itemize}
\end{frame}

\begin{frame}[plain,allowframebreaks]\small\frametitle{Velocity of a curve in $\overset{\circ}\Pi(q_1,q_2)$}
...
$      t \mapsto q(t) \in \overset{\circ}{\Pi}(q_1,q_2)
 $.
    ...
    \end{gather*}
    \item For each random variable depending on one variable only, $f_j\circ X_j$, $j = 1,2$, it holds
      ...
      It follows that \emph{$\condexpat {q(t)} {\velocity q(t)} {X_j} = 0$}.
      \item
That is, \emph{if $t \mapsto q(t) \in \openplan{q_1}{q_2}$, then $\velocity q(t)$ is an interaction at $q(t)$, $\velocity q(t) \in H_2(q(t))$}.
\item Conversely, let $q \in \openplan{q_1}{q_2}$ and $c_{12} \in H_q(q)$. The curve $t \mapsto (1+tc) \cdot q$ is defined for $t$ in a neighborhood of 0. The margins are correct,
\begin{equation*}
...
\item In conclusion, for all $q \in \overset{\circ}{\Pi}(q_1,q_2)$, the velocities fibre is the vector space of interactions,
\begin{equation*}
...
\end{equation*}
where $q \in \openplan{q_1}{q_2}$.

\item The complement fiber is 
\begin{multline*}
