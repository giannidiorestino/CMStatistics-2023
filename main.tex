...
\title{\it Information Geometry of ANOVA and Transport}
...
\begin{document} 
% 
\begin{frame}\frametitle{DISMA POLITO June 20, 2023}  
...
\emph{Functional ANOVA} (Analysis of variance) appears in Statistics and System Theory. It is a particular orthogonal splitting of the vector space of square-integrable random variables. When the sample space is factorial,  it conveniently splits the fibres of the affine bundle consisting of couples of probability functions and Fisher's scores, which we call the \emph{Statistical Bundle}. One of the terms in the splitting is the \emph{additive model}, while the other is related to the \emph{transportation model} with fixed marginals. It is a concept known in the classical theory of contingency tables. We rephrase and show implications to algebraic statistics, information geometry, and Kantorovich optimal transport. In this setting, the \emph{gradient flow} in the transport sub-model has a limit point that solves the \emph{Kantorovich problem}.
...
\begin{frame}[plain]\small\frametitle{Keywords explained}

Functional ANOVA (ANalysis Of VAriance) is used in statistics and sistem theory. It is a non-parametric  orthogonal splitting of the vector space of $L^2$ random variables. We use it to split the \emph{fibres of the affine bundle} of couples of probability densities and Fisher's scores, the statistical bundle, on a product sample space. One of the factors is an additive model, while the other factor is a transport model. 
...
\end{frame}
    ...
    \end{equation*}
    \item \emph{We focus on the case $q \neq q_1 \otimes q_2$}.
    ...
    \end{equation*}
    \item Consider the subsets of $\set{1,2}$, that is $\emptyset, \set{1},\set{2},\set{1,2}$, partially ordered by inclusion. Let $X_I$ be the components projection on $I$, $X_I = (X_j \colon j \in I)$. Each $I$ is an \emph{interaction}. 
    ...
    \item The \emph{order} of the interaction $I$ is $\# I$. Let $H_k$ be the vector space generated by the $I$-interactions of order $k$.
      \begin{itemize}
   ...
   \end{equation*}
    ...
    \item Let $f \mapsto \hajekof q f$ be the orthogonal projection of $L^2(q)$ onto $H_1$, the \emph{Hajek projection}. It is a general device to approximate a complex random variable with an additive model, cf Hajek:1968 and Efron-Stein:1981.\footnote{\tiny I suspect that it is what some call mean field approximation.}
    \item Then, the orthogonal decomposition is 
    \begin{equation*}
        f = \Expectation_q f + \hajekof q f + (\operatorname I  - 
 ...
 \begin{align*}
        ...
        \end{align*} 
\item The decomposition becomes
\begin{multline*}
f = \expectat q f + \\
...
\item Write $q(x_1,x_2) = k(x_1,x_2) q_1(x_1) q_2(x_2)$, that is,
\begin{align*}
    q_{1|2}(x_1|x_2) = k(x_1,x_2)q_1(x_1) \\
       q_{2|1}(x_2|x_1) = k(x_1,x_2)q_2(x_2)
\end{align*}
\item The Bayes rule for conditional expectation is (see exercise)
\begin{equation*}
    \condexpat q g Z = \frac{\condexpat {q_1 \otimes q_2} {kg} Z}{\condexpat {q_1 \otimes q_2} {k} Z}
\end{equation*}
\item Especially,
\begin{multline*}
   \condexpat q g {X_1=x_1} = \sum_y g(x_1,y) q_{2|1}(y|x_1) \\ = \sum_y g(x_1,y) \, k(x_1,y)q_2(y)
\end{multline*}
and
\begin{multline*}
   \condexpat q g {X_2=x_2} = \sum_x g(x,x_2) q_{1|2}(x|x_2) \\ = \sum_x g(x,x_2) \, k(x,x_2) q_1(x)
   \end{multline*}
\end{itemize}

   \end{frame}

   \begin{frame}[plain,allowframebreaks]\frametitle{Computing the Hajek projection}
 \begin{itemize}      
 
\item The equations above allow for writing a componentwise system of linear equations, cf Pistone 2021.

\item If we assume $\expectat q f = 0$, the gradient equations for the Hajek projection are

\begin{align*}
        \condexpat q {f}{X_1} &= f_1\circ X_1 + \condexpat q {f_2 \circ X_2}{X_1} \\ 
            \condexpat q {f}{X_2} &=  \condexpat q {f_1 \circ X_1}{X_2} + f_2\circ X_2
        \end{align*}
\item With the kernel expression $q = k \cdot  q_1 \otimes q_2$
\begin{align*}
    \sum_y f(x_1,y) \, k(x_1,y) q_2(y) &= f_1(x_1) + \sum_y f_2(y) \, k(x_1,y) q_2(y) \ , \\ \sum_x f(x,x_2) \, k(x,x_2) q_1(x) 
    &= \sum_x f_1(x) \, k(x,x_2) q_1(x) + f_2(x_2) \ .
\end{align*}
